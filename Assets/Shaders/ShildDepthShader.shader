    ...
    SubShader
    {
		...
		Tags{
			...
			Varyings LitPassVertex(Attributes input){
				...
				output.ssPos.xy=output.positionCS.xy*0.5+0.5*float2(output.positionCS.w,output.positionCS.w);
			...
			float4 LitPassFragment(Varyings input):SV_TARGET{
				...
				float4 depthColor=SAMPLE_DEPTH_TEXTURE(_CameraDepthTexture,sampler_CameraDepthTexture,input.ssPos.xy);
				float depthBuffer=LinearEyeDepth(depthColor,_ZBufferParams);

				float depth=input.ssPos.w-_depthoffset;
				

				//float edgeLight=saturate(depth-depthBuffer+0.005)*100*_depthoffset;
				float edgeLight=saturate(1-abs(depth-depthBuffer));
				edgeLight=smoothstep(0,1,edgeLight);


				float NdotV=abs(dot(N,V));
				float color=FresnelSchlick(NdotV,0.8).r;

				float flow=saturate(
				pow(
				1-
				abs(
				frac(input.positionWS.y*0.25-_Time.y*0.3)-0.5
				)
				,10
				)
				*0.3
				);
				float4 flowColor=flow*_BaseColor;

				return float4(_BaseColor.xyz,edgeLight*color)+flowColor;
